...
def main() -> None:
    ...
    export_table(policy_table, table_dir / "表11-1_政策事实表")
    ...
    export_table(support, table_dir / "表11-2_处理批次支持域")
    ...
    export_table(pilot_support, table_dir / "表11-3_各试点样本覆盖")
    ...
    export_table(design_checks, table_dir / "表11-4_研究设计验收")

    threats = pd.DataFrame(
        [
            ["选择性获批", "政策前特征与趋势", "描述统计、平衡检验、交错DID", "第12、13章"],
            ["政策预期", "政策前事件时间系数", "事件研究与联合检验", "第13章"],
            ["批复年份错配", "批复当年与完整年度", "替代处理起点", "第13章"],
            ["疫情冲击", "2020年与特殊城市", "排除年份和城市", "第13章"],
            ["同期政策", "教学代理变量", "控制或剔除代理重叠样本", "第13章"],
            ["地址错配", "教学办公地址代理", "替代处理时点", "第13章"],
            ["空间溢出", "同省近邻教学代理", "剔除近邻对照组", "第13章"],
            ["少量处理范围", "18个试点范围", "城市聚类与留一试点", "第13章"],
        ],
        columns=["识别威胁", "所需证据", "对应检验", "落实章节"],
    )
    export_table(threats, table_dir / "表11-5_识别威胁与证据地图")
    ...
    save_figure(fig, figure_dir / "图11-1_试验区批复时间线")
    ...
    axis.grid(axis="x")
    axis.grid(axis="y", visible=False)
    save_figure(fig, figure_dir / "图11-2_各试点企业覆盖")

    summary = f"""# 第11章案例结果说明
...
处理状态通过唯一键、单调性和支持域检查。样本中有{treated['firm_id'].nunique():,}家处理企业、{data.loc[data['did_treated'].eq(0), 'firm_id'].nunique():,}家从未处理企业，城市聚类数为{data['analysis_city_id'].nunique()}。各年度处理组均具有多个政策前年份和至少两个政策后年份，可以进入第13章的动态分析。

第11章只确认研究问题、政策事实、处理时点和识别威胁，不在研究设计阶段使用显著性倒推模型。同期政策、办公地址和空间距离在当前教学数据中没有真实观测，第13章只能使用明确标注的教学代理变量演示代码路径。
"""
    (note_dir / "第11章案例结果说明.md").write_text(summary, encoding="utf-8")
