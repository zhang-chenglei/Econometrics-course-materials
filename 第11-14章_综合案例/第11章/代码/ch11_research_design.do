...
capture log close
log using "$PKG/第11章/说明/ch11_stata.log", text replace
...
export delimited using "$PKG/第11章/表格/stata_处理批次年度支持.csv", replace
restore
...
export delimited using "$PKG/第11章/表格/stata_各试点覆盖企业数.csv", replace
graph hbar firms, over(policy_scope_name, sort(1) descending label(labsize(vsmall))) ///
    ytitle("企业数") title("各试点在教学样本中的企业覆盖")
graph export "$PKG/第11章/图形/stata_图11-2_各试点企业覆盖.png", width(2200) replace
restore
