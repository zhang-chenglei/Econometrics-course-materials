...
def main() -> None:
    ...
    export_table(sample_flow, table_dir / "表12-1_样本筛选流程")
    ...
    export_table(descriptive, table_dir / "表12-2_描述性统计")
    ...
    export_table(missingness, table_dir / "表12-3_主要分析变量缺失率")
    ...
    export_table(balance, table_dir / "表12-4_政策前特征平衡")
    ...
    export_table(batch_support, table_dir / "表12-5_年度处理组样本覆盖")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图12-1_主要结果变量分布")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图12-2_处理组与对照组原始趋势")
    ...
    save_figure(fig, figure_dir / "图12-3_政策前特征平衡图")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图12-4_处理组支持热图")
...
政策前平衡表显示，处理组和对照组的最大绝对标准化差异为{max_abs_smd:.3f}。这些差异用于说明试点并非随机分配，不能把描述性平衡直接当作平行趋势成立。第13章将通过交错DID动态估计和政策前联合检验进一步检查识别条件。
"""
