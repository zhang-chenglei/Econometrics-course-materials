...
CH13_TABLES = ROOT / "第13章" / "表格"
TABLE_DIR = ROOT / "第14章" / "表格"
FIGURE_DIR = ROOT / "第14章" / "图形"
NOTE_DIR = ROOT / "第14章" / "说明"
...
def read_result(name: str) -> pd.DataFrame:
    return pd.read_csv(CH13_TABLES / name, encoding="utf-8-sig")
...
twfe = read_result("表13-1_TWFE教学基准.csv")
staggered = read_result("表13-2_交错DID总体ATT.csv")
event = read_result("表13-3_动态事件研究.csv")
pretrend = read_result("表13-5_平行趋势联合检验.csv")
robust = read_result("表13-6_稳健性检验.csv")
hetero = read_result("表13-8_异质性直接差异检验.csv")
mechanism_twfe = read_result("表13-10_机制分析.csv")
mechanism = read_result("表13-10b_机制交错DID.csv")
data = load_data()


# ── 表14-1：TWFE教学基准（与教材中的四列表一致） ──────────────────────────────
...
save_table(table_14_1, TABLE_DIR / "表14-1_TWFE教学基准最终版")
...
save_table(table_14_2, TABLE_DIR / "表14-2_交错DID主结果最终版")
...
save_table(table_14_3, TABLE_DIR / "表14-3_关键稳健性结果")


# ── 表14-4、14-5：异质性直接差异与机制证据 ────────────────────────────────────
...
save_table(table_14_4, TABLE_DIR / "表14-4_异质性直接差异检验")
...
save_table(table_14_5, TABLE_DIR / "表14-5_机制相关证据")
...
save_table(audit, TABLE_DIR / "表14-6_全文关键数字一致性检查")
...
fig.tight_layout()
save_figure(fig, FIGURE_DIR / "图14-1_主结果比较")
...
fig.tight_layout()
save_figure(fig, FIGURE_DIR / "图14-2_事件研究最终版")
...
fig.tight_layout()
save_figure(fig, FIGURE_DIR / "图14-3_稳健性结果摘要")
...
axis.grid(axis="x", alpha=0.22)
fig.tight_layout()
save_figure(fig, FIGURE_DIR / "图14-4_机制证据摘要")
...
样本为{int(data.year.min())}—{int(data.year.max())}年企业面板，包含{data.firm_id.nunique():,}家企业、{data.analysis_city_id.nunique():,}个城市和{len(data):,}个观测。结果变量为研发投入强度和发明专利申请数的 `ln(1+x)` 变换。企业层面控制变量包括规模、资产负债率、盈利能力、成长性、固定资产比重、现金流和企业年龄。完整变量、缺失率、样本筛选与平衡结果见第12章输出。
...
图14-2以处理前一年为基准，展示政策前后事件时间聚合ATT。研发投入和发明专利结果的政策前系数联合检验p值分别为{rd_pre_p:.3f}和{pat_pre_p:.3f}，在本教学样本中没有拒绝政策前动态共同为零。改用尚未处理企业作对照后，研发结果仍通过联合检验，但专利结果的p值为{not_yet_pat_p:.3f}。这说明研究结论的可信度不仅取决于点估计，也取决于对照组口径与识别诊断。
...
- 图14-4：机制证据摘要
"""
(NOTE_DIR / "第14章完整研究报告_半合成教学示范.md").write_text(report, encoding="utf-8")
