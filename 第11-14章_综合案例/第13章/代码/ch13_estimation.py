...
def main() -> None:
    ...
    export_table(twfe_table, table_dir / "表13-1_TWFE教学基准")
    ...
    export_table(overall_table, table_dir / "表13-2_交错DID总体ATT")
    export_table(event_table, table_dir / "表13-3_动态事件研究")
    export_table(cohort_table, table_dir / "表13-4_年度处理组效应")
    ...
    export_table(pretrend, table_dir / "表13-5_平行趋势联合检验")
    ...
    export_table(robustness_table, table_dir / "表13-6_稳健性检验")
    ...
    export_table(leave_one, table_dir / "表13-7_留一试点影响力诊断")
    ...
    export_table(interaction_table, table_dir / "表13-8_异质性直接差异检验")
    export_table(group_effects, table_dir / "表13-9_异质性分组效应")
    ...
    export_table(mechanism_table, table_dir / "表13-10_机制分析")
    ...
    export_table(evidence_map, table_dir / "表13-11_证据地图")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-1_动态事件研究")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-2_年度处理组效应")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-3_稳健性系数图")
    ...
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-4_留一试点影响力诊断")
    ...
    fig.text(
        0.5,
        -0.01,
        "是否存在组间差异以表13-8中的直接交互项检验为准",
        ha="center",
        color=GREY,
        fontsize=9.5,
    )
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-5_异质性系数图")
    ...
    axis.text(
        0,
        1.01,
        "虚线为|t|=1.96；金色表示5%水平显著；正式机制结果采用从未处理组的交错DID，见表13-10b",
    ...
    axis.grid(axis="y", visible=False)
    fig.tight_layout()
    save_figure(fig, figure_dir / "图13-6_机制结果")
