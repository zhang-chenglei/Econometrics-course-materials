...
* 任务1：成绩水平模型与对数成绩模型
regress score ai age i.strong
estimates store level
regress ln_score ai age i.strong
estimates store log

* 任务2：AI 使用时间的二次项
regress ln_score ai c.ai##c.ai age i.strong
estimates store quadratic
display "对数成绩模型中 AI 的极值点 = " -_b[ai]/(2*_b[c.ai#c.ai])
margins, at(ai=(0(2)24) age=(20) strong=(0))
...
* 任务3：AI 使用时间与基础好/基础弱的交互项
regress ln_score c.ai##i.strong c.ai##c.ai age
estimates store interaction
margins strong, dydx(ai)
margins strong, dydx(ai) at(ai=(5 10 15 20))
margins strong, at(ai=(0(2)24) age=(20))
marginsplot, recast(line) recastci(rarea) ///
    title("AI 使用与成绩：基础好与基础弱的两条预测曲线") ///
    xtitle("每周 AI 使用时间（小时）") ytitle("预测对数成绩")
graph export output/ch04-fig8-ai-base.png, width(1800) replace
