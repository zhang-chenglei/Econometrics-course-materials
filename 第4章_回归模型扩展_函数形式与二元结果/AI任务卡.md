...
在同一个AI与成绩案例中依次理解对数、AI使用时间的平方项、“AI×基础好”交互项，以及连续成绩与二元达标结果回答的问题有何不同。
...
4. 最终模型同时含`ai²`和`ai×strong`；在代表性AI取值处计算两组边际效应及置信区间，并报告组别AME。
...
- 含平方项和交互项时，完整边际效应为$\beta_{ai}+2\beta_{ai^2}ai+\beta_{ai\times strong}strong$；
- $\beta_{ai}+\beta_{ai\times strong}$只对应基础弱在`ai=0`处的局部斜率，不能称为基础弱在所有AI水平上的恒定斜率；
