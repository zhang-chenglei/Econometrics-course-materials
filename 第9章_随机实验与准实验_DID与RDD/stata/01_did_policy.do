...
graph export output/ch09-fig1-parallel-trend.png, width(2400) replace
restore
...
estimates restore m_did_base
local b_base  = _b[1.treated#1.post]
local se_base = _se[1.treated#1.post]
estimates restore m_did_ctrl
local b_ctrl  = _b[1.treated#1.post]
local se_ctrl = _se[1.treated#1.post]
estimates restore m_did_fe
local b_fe    = _b[did]
local se_fe   = _se[did]
...
estimates store m_event

display _n "--- 事件研究系数 ---"
foreach v in did_pre3 did_pre2 did_post0 did_post1 did_post2 did_post3 {
    display "  `v': " %9.4f _b[`v'] " (se=" %9.4f _se[`v'] ", p=" %9.4f 2*ttail(e(df_r), abs(_b[`v']/_se[`v'])) ")"
}
display "  政策前（did_pre3, did_pre2）→ 期望不显著；但**不显著不等于平行趋势成立**"
display "  政策后（did_post0~did_post3）→ 应显著为正（政策效应）"
display "  如果政策前系数显著 → 平行趋势假设受质疑 → DiD结论必须重新论证"
...
local b_fake  = _b[fake_did]
local se_fake = _se[fake_did]
...
local b_rand  = _b[random_did]
local se_rand = _se[random_did]
...
* ============================================================
* 落盘：把关键系数写成 CSV，供正文数字逐项对照
* ============================================================
tempname fh
file open `fh' using "output/ch09_did_three_specs.csv", write replace
file write `fh' "设定,DiD系数,标准误" _n
file write `fh' "基准交互项,"    %9.4f (`b_base') "," %9.4f (`se_base') _n
file write `fh' "加控制变量,"    %9.4f (`b_ctrl') "," %9.4f (`se_ctrl') _n
file write `fh' "企业FE+年份FE," %9.4f (`b_fe')   "," %9.4f (`se_fe')   _n
file close `fh'

tempname fh2
file open `fh2' using "output/ch09_event_study.csv", write replace
file write `fh2' "项,系数,标准误" _n
estimates restore m_event
foreach v in did_pre3 did_pre2 did_post0 did_post1 did_post2 did_post3 {
    file write `fh2' ("`v'") "," %9.4f (_b[`v']) "," %9.4f (_se[`v']) _n
}
file close `fh2'

tempname fh3
file open `fh3' using "output/ch09_placebo_tests.csv", write replace
file write `fh3' "检验,DiD系数,标准误" _n
file write `fh3' "时间安慰剂(假政策时点2019)," %9.4f (`b_fake') "," %9.4f (`se_fake') _n
file write `fh3' "空间安慰剂(随机假处理组)," %9.4f (`b_rand') "," %9.4f (`se_rand') _n
file close `fh3'
