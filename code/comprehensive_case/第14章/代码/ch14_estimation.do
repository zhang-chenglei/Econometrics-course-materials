...
capture log close
log using "$PKG/第14章/说明/ch14_stata.log", text replace
...
eststo pat_ctrl
esttab rd_base rd_ctrl pat_base pat_ctrl using "$PKG/第14章/表格/stata_TWFE教学基准.csv", ///
...
graph export "$PKG/第14章/图形/stata_图14-1a_研发投入动态效应.png", width(2200) replace
...
graph export "$PKG/第14章/图形/stata_图14-1b_专利动态效应.png", width(2200) replace
...
esttab using "$PKG/第14章/表格/stata_关键稳健性.csv", ///
...
esttab using "$PKG/第14章/表格/stata_机制相关结果.csv", ///
    keep(did_post_full) b(4) se(4) star(* 0.10 ** 0.05 *** 0.01) ///
    stats(N) replace csv
