...
CONFIG = {
    12: {
        "title": "第12章：从政策事实到研究设计",
        "goal": "核对政策批次、样本覆盖和识别威胁，把研究问题变成可执行的设计书。",
        "script": "ch12_research_design.py",
        "tables": ["表12-1_政策事实表.csv", "表12-4_研究设计验收.csv", "表12-5_识别威胁与证据地图.csv"],
        "figures": ["图12-1_试验区批复时间线.png", "图12-2_各试点企业覆盖.png"],
    },
    13: {
        "title": "第13章：数据准备与变量构建",
        "goal": "检查唯一性、缺失、分布、处理支持和平衡性，生成可进入估计环节的分析数据诊断。",
        "script": "ch13_data_preparation.py",
        "tables": ["表13-1_样本筛选流程.csv", "表13-2_描述性统计.csv", "表13-4_政策前特征平衡.csv"],
        "figures": ["图13-1_主要结果变量分布.png", "图13-2_处理组与对照组原始趋势.png", "图13-3_政策前特征平衡图.png"],
    },
    14: {
        "title": "第14章：模型估计与证据检验",
        "goal": "依次完成TWFE教学基准、交错DID、动态效应、稳健性、异质性和机制相关检验。",
        "script": "ch14_estimation.py",
        "tables": ["表14-1_TWFE教学基准.csv", "表14-2_交错DID总体ATT.csv", "表14-5_平行趋势联合检验.csv", "表14-10_机制分析.csv", "表14-10b_机制交错DID.csv"],
        "figures": ["图14-1_动态事件研究.png", "图14-3_稳健性系数图.png", "图14-5_异质性系数图.png", "图14-6_机制结果.png"],
    },
    15: {
        "title": "第15章：研究写作与成果呈现",
        "goal": "把前章输出整理为统一数字口径的论文式表图和一份克制、可追溯的教学研究报告。",
        "script": "ch15_reporting.py",
        "tables": ["表15-1_TWFE教学基准最终版.csv", "表15-2_交错DID主结果最终版.csv", "表15-6_全文关键数字一致性检查.csv"],
        "figures": ["图15-1_主结果比较.png", "图15-2_事件研究最终版.png", "图15-3_稳健性结果摘要.png", "图15-4_机制证据摘要.png"],
    },
}
...
for chapter, config in CONFIG.items():
    ...
    notebook["cells"] = [
        ...
        nbf.v4.new_code_cell(
            ...
            "if (p / '共享代码').is_dir() and (p / '第12章').is_dir()), None)\n"
            "if PACKAGE is None:\n"
