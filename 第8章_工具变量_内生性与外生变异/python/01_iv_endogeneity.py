...
comparison_1 = pd.DataFrame({
    'OLS系数': m_ols.params.round(4),
    'OLS标准误': m_ols.bse.round(4),
    'IV系数': m_iv.params.round(4),
    'IV标准误': m_iv.std_errors.round(4),
})
print(comparison_1)
comparison_1.to_csv(OUTPUT_DIR / "ch08_ols_vs_iv.csv", encoding="utf-8-sig")
...
# 落盘：第一阶段系数与偏F，便于与正文数字逐项对照
pd.DataFrame({
    'coef': m_stage1.params,
    'std_err': m_stage1.bse,
    't': m_stage1.tvalues,
    'p_value': m_stage1.pvalues,
}).round(4).to_csv(OUTPUT_DIR / "ch08_first_stage.csv", encoding="utf-8-sig")
pd.DataFrame({'partial_F': [f_stat]}).round(4).to_csv(
    OUTPUT_DIR / "ch08_first_stage_f.csv", index=False, encoding="utf-8-sig")
...
# 用纯噪声工具做一次 2SLS：估计值会剧烈跳动、置信区间极宽
m_iv_weak = IV2SLS(df['ROE'], X_iv, df['RD'], noise_iv).fit(cov_type='robust')
ci_weak = m_iv_weak.conf_int().loc['RD']
ci_strong = m_iv.conf_int().loc['RD']
print(f'  弱工具2SLS的RD系数: {m_iv_weak.params["RD"]:.3f}  标准误: {m_iv_weak.std_errors["RD"]:.3f}'
      f'  95%区间: [{ci_weak.iloc[0]:.2f}, {ci_weak.iloc[1]:.2f}]')
print(f'  强工具IV的RD系数:   {m_iv.params["RD"]:.3f}  标准误: {m_iv.std_errors["RD"]:.3f}'
      f'  95%区间: [{ci_strong.iloc[0]:.2f}, {ci_strong.iloc[1]:.2f}]')
print('  → 弱工具把置信区间撑得极宽，点估计也不再可信：这是弱识别风险的直观表现')

pd.DataFrame({
    'RD系数': [m_iv_weak.params['RD'], m_iv.params['RD']],
    '标准误': [m_iv_weak.std_errors['RD'], m_iv.std_errors['RD']],
    'CI下限': [ci_weak.iloc[0], ci_strong.iloc[0]],
    'CI上限': [ci_weak.iloc[1], ci_strong.iloc[1]],
}, index=['纯噪声工具(弱)', 'ResDensity(强)']).round(4).to_csv(
    OUTPUT_DIR / "ch08_weak_vs_strong_iv.csv", encoding="utf-8-sig")
...
print(comparison_3)
comparison_3.to_csv(OUTPUT_DIR / "ch08_single_vs_two_iv.csv", encoding="utf-8-sig")
...
pd.DataFrame({'statistic': [overid.stat], 'p_value': [overid.pval]}).round(4).to_csv(
    OUTPUT_DIR / "ch08_overid.csv", index=False, encoding="utf-8-sig")

print('\n=== 提示 ===')
print('  H0: 过度识别约束成立（解释依赖至少有足够有效工具等前提）')
print('  p > 0.05 → 不拒绝H0，但不能证明所有工具都外生')
print('  p < 0.05 → 至少某些工具或模型约束与数据不相容')
print('      （可能是工具无效、模型设定错误，或不同工具识别了不同参数）')
...
pd.DataFrame({'t': [dwh_t], 'p_value': [dwh_pval]}).round(4).to_csv(
    OUTPUT_DIR / "ch08_dwh_endogeneity.csv", index=False, encoding="utf-8-sig")
...
summary = pd.DataFrame({
    'OLS系数': m_ols.params[['RD', 'size', 'age']].round(4),
    'OLS标准误': m_ols.bse[['RD', 'size', 'age']].round(4),
    'IV(Z1)系数': m_iv.params[['RD', 'size', 'age']].round(4),
    'IV(Z1)标准误': m_iv.std_errors[['RD', 'size', 'age']].round(4),
    'IV(Z1+Z2)系数': m_iv2.params[['RD', 'size', 'age']].round(4),
    'IV(Z1+Z2)标准误': m_iv2.std_errors[['RD', 'size', 'age']].round(4),
})
print(summary)
summary.to_csv(OUTPUT_DIR / "ch08_summary_three_models.csv", encoding="utf-8-sig")
print(f'\n结果已保存至：{OUTPUT_DIR}')
