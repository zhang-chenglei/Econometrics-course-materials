...
ivregress 2sls ROE (RD = ResDensity) size age, robust
estat firststage
local fs_F   = r(singleresults)[1,4]
local fs_df2 = r(singleresults)[1,6]
...
local weak_b  = _b[RD]
local weak_se = _se[RD]
estat firststage
local weak_F = r(singleresults)[1,4]
drop noise_iv

display _n "  上面的F值应远低于预警线10（无关工具的F期望约为1）——演示什么是'弱工具'"
...
* 过度识别检验（此处为异方差稳健的得分检验；同方差下才是 Sargan）
estat overid
local oid_stat = r(score)
local oid_df   = r(df)
local oid_p    = r(p_score)
local oid_p    = r(p_score)
...
display "  过度识别检验的必要前提：至少有一个工具有效，且模型设定正确"
...
estat endogenous
local dwh_F  = r(regF)
local dwh_df = r(regFdf_n)
local dwh_p  = r(p_regF)
...
* ============================================================
* 落盘：把关键系数与检验统计量写成 CSV，供正文数字逐项对照
* ============================================================
tempname fh
file open `fh' using "output/ch08_model_comparison.csv", write replace
file write `fh' "模型,RD,RD标准误,size,size标准误,age,age标准误" _n
foreach m in m_ols m_iv m_iv2 {
    estimates restore `m'
    file write `fh' ("`m'")
    foreach v in RD size age {
        file write `fh' "," %9.4f (_b[`v']) "," %9.4f (_se[`v'])
    }
    file write `fh' _n
}
file close `fh'
...
tempname fh2
file open `fh2' using "output/ch08_diagnostics.csv", write replace
file write `fh2' "检验,统计量,自由度,p值" _n
file write `fh2' "强工具第一阶段F," %9.4f (`fs_F') ",," _n
file write `fh2' "弱工具第一阶段F(噪声工具)," %9.4f (`weak_F') ",," _n
file write `fh2' "过度识别(Wooldridge稳健得分)," %9.4f (`oid_stat') "," %9.1f (`oid_df') "," %9.4f (`oid_p') _n
file write `fh2' "Durbin-Wu-Hausman内生性(F)," %9.4f (`dwh_F') "," %9.1f (`dwh_df') "," %9.4f (`dwh_p') _n
file close `fh2'

tempname fh3
file open `fh3' using "output/ch08_weak_iv_2sls.csv", write replace
file write `fh3' "RD系数,标准误" _n
file write `fh3' %9.4f (`weak_b') "," %9.4f (`weak_se') _n
file close `fh3'
