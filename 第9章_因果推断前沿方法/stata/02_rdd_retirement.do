...
display "  假断点58岁: D系数 = " %9.2f _b[fake58] " (p=" %6.4f (2*ttail(e(df_r),abs(_b[fake58]/_se[fake58]))) ")"

gen running62 = age - 62
gen fake62 = (age >= 62)
reg consumption running62 fake62 c.running62#c.fake62 if age > 60 & abs(running62) <= 5, robust
display "  假断点62岁: D系数 = " %9.2f _b[fake62] " (p=" %6.4f (2*ttail(e(df_r),abs(_b[fake62]/_se[fake62]))) ")"
...
display "  协变量未见明显跳跃，与连续性相容；较大的p值不能证明个体必然可比"
display "  假断点未见明显跳跃可作辅助证据，但不能单独证明政策效应"
display ""
display "  RDD标准流程："
