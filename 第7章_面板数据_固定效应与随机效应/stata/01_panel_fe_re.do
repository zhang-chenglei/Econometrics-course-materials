...
* ============================================================
* 落盘：把各模型的关键系数写成 CSV，供正文数字逐项对照
* ============================================================
tempname fh
file open `fh' using "output/ch07_model_comparison.csv", write replace
file write `fh' "模型,RD,RD标准误,assets,assets标准误,debt,debt标准误" _n
foreach m in m_pooled m_fe m_re m_twoway {
    estimates restore `m'
    file write `fh' ("`m'")
    foreach v in RD assets debt {
        file write `fh' "," %9.5f (_b[`v']) "," %9.5f (_se[`v'])
    }
    file write `fh' _n
}
file close `fh'

* 三种FE方法（系数名不同：RD 与 d_RD）
estimates restore m_within
local b_within = _b[RD]
local se_within = _se[RD]
estimates restore m_lsdv
local b_lsdv = _b[RD]
local se_lsdv = _se[RD]
estimates restore m_fd
local b_fd = _b[d_RD]
local se_fd = _se[d_RD]
tempname fh2
file open `fh2' using "output/ch07_three_fe_methods.csv", write replace
file write `fh2' "方法,RD系数,标准误" _n
file write `fh2' "组内去心法," %9.6f (`b_within') "," %9.6f (`se_within') _n
file write `fh2' "LSDV法,"     %9.6f (`b_lsdv')   "," %9.6f (`se_lsdv')   _n
file write `fh2' "一阶差分法," %9.6f (`b_fd')     "," %9.6f (`se_fd')     _n
file close `fh2'

* 豪斯曼检验统计量
tempname fh3
file open `fh3' using "output/ch07_hausman.csv", write replace
file write `fh3' "chi2,df,p_value" _n
file write `fh3' %9.4f (`hausman_chi2') ",1," %9.4f (`hausman_p') _n
file close `fh3'
