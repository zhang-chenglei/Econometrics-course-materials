...
# 需要把结果汇入教材案例结果库时，运行时指定：
#   CASE_OUTPUT_DIR=<仓库>/30_教学/09_计量教材/案例结果 python 01_panel_fe_re.py
...
print(comparison_1)
comparison_1.to_csv(OUTPUT_DIR / "ch07_pooled_vs_fe.csv", encoding="utf-8-sig")
...
three_fe = pd.DataFrame({
    'RD系数': [rd_within, rd_lsdv, rd_fd],
}, index=['组内去心法', 'LSDV法', '一阶差分法']).round(6)
three_fe.to_csv(OUTPUT_DIR / "ch07_three_fe_methods.csv", encoding="utf-8-sig")
...
print(comparison_3)
comparison_3.to_csv(OUTPUT_DIR / "ch07_fe_vs_re.csv", encoding="utf-8-sig")
...
print(comparison_4)
comparison_4.to_csv(OUTPUT_DIR / "ch07_pooled_fe_twoway.csv", encoding="utf-8-sig")
...
summary.to_csv(OUTPUT_DIR / "ch07_summary_four_models.csv", encoding="utf-8-sig")

# 豪斯曼检验的统计量单独存一份，便于与正文数字逐项对照
pd.DataFrame({
    'statistic': [hausman_stat],
    'df': [hausman_df],
    'p_value': [hausman_p],
}).round(6).to_csv(OUTPUT_DIR / "ch07_hausman.csv", index=False, encoding="utf-8-sig")
