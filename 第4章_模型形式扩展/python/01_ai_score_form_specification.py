...
教学用模拟数据，数据生成过程人为设定且已知：
    ...
    score  = 56 + 2.0*ai - 0.060*ai^2 + 0.55*(ai × female)
...
- ch04_group_slopes.csv            男女两组的 AI 斜率及其置信区间
- ch04-fig2-quadratic.png          二次项示意图
- ch04-fig3-interaction.png        交互项示意图
- ch04-fig4-ai-score-curve.png     案例：AI 使用时间与预测成绩
- ch04-fig5-ai-gender.png          案例：男女两组的 AI 使用—成绩预测线
"""
...
male_se = np.sqrt(cov.loc["ai", "ai"])
female_se = np.sqrt(
    cov.loc["ai", "ai"]
    + cov.loc["ai_female", "ai_female"]
    + 2 * cov.loc["ai", "ai_female"]
)
...
group_slopes = pd.DataFrame(
    {
        "group": ["男性", "女性"],
        "ai_slope": [male_slope, female_slope],
        "std_err": [male_se, female_se],
    }
)
group_slopes["ci_lower"] = group_slopes.ai_slope - 1.96 * group_slopes.std_err
group_slopes["ci_upper"] = group_slopes.ai_slope + 1.96 * group_slopes.std_err
group_slopes.to_csv(TABLE_DIR / "ch04_group_slopes.csv", index=False, encoding="utf-8-sig")
...
ax.legend(frameon=False)
fig.tight_layout()
fig4 = OUTPUT_DIR / "ch04-fig4-ai-score-curve.png"
fig.savefig(fig4, dpi=220, bbox_inches="tight")

# 图4-5 案例：男女两组的 AI 使用—预测对数成绩线
fig, ax = plt.subplots(figsize=(8, 4.8))
for female_value, label, color in [(0, "男性", BLUE), (1, "女性", "#D17B88")]:
    pred = pd.DataFrame(
        {
...
ax.set(title="AI 使用与成绩：不同性别的两组斜率",
       xlabel="每周 AI 使用时间（小时）", ylabel="预测对数成绩")
ax.legend(frameon=False)
fig.tight_layout()
fig5 = OUTPUT_DIR / "ch04-fig5-ai-gender.png"
