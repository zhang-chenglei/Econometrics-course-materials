...
- ch05_nonlinear_regression.csv    恢复二次项和交互项后的HC1稳健结果
- ch05_nonlinear_marginal_effects.csv 正确非线性模型的代表性边际效应
- ch05_nonlinear_group_ame.csv     正确非线性模型的分组AME
- ch05_diagnostic_checklist.csv    四列诊断清单
- ch05-fig2-fwl-net-variation.png  净化后的 AI 与净化后的成绩
- ch05-fig4-residual-heteroskedasticity.png 残差图
"""
...
# RESET发出函数形式警报后，恢复第4章的二次项和交互项。
# 这一模型同时纳入公开DGP中的控制变量，并用HC1处理异方差推断。
nonlinear_vars = [
    "ai",
    "age",
    "female",
...
nonlinear_table = pd.DataFrame(
    {
        "coef": nonlinear.params,
        "robust_se_HC1": nonlinear.bse,
        "ci_lower": nonlinear.conf_int()[0],
        "ci_upper": nonlinear.conf_int()[1],
    }
)
nonlinear_table.to_csv(TABLE_DIR / "ch05_nonlinear_regression.csv", encoding="utf-8-sig")
...
nonlinear_me = pd.DataFrame(nonlinear_me_rows)
nonlinear_me.to_csv(
    TABLE_DIR / "ch05_nonlinear_marginal_effects.csv", index=False, encoding="utf-8-sig"
)
...
nonlinear_ame = pd.DataFrame(nonlinear_ame_rows)
nonlinear_ame.to_csv(
    TABLE_DIR / "ch05_nonlinear_group_ame.csv", index=False, encoding="utf-8-sig"
)
...
print("\n恢复二次项和交互项后的代表性边际效应（HC1）：")
print(nonlinear_me.round(5).to_string(index=False))
print("\n恢复后模型的分组AME（HC1）：")
print(nonlinear_ame.round(5).to_string(index=False))
...
fig.tight_layout()
output = OUTPUT_DIR / "ch05-fig4-residual-heteroskedasticity.png"
...
fig.tight_layout()
output_fwl = OUTPUT_DIR / "ch05-fig2-fwl-net-variation.png"
